...
function lyapunov_direct(γs, ws, u0; σ=10.0, β=8/3, ρ0=28.0, tspan=(0.0, 120.0), transient=30.0, dt_renorm=1.0, seed=1234)
...
begin
    # Pack parameters in log space (8 params: σ, β, ρ0, γ1, γ2, γ3, w1, w2, w3)
    function pack(σ, β, ρ0, γs, ws)
        L = length(γs)
        ...
        # Pack as: log(σ-8), log(β-2.5), log(ρ0-20), log(dγ), log(ws)
        vcat(log(max(σ-8, 1e-12)), log(max(β-2.5, 1e-12)), log(max(ρ0-20, 1e-12)), log.(dγ), log.(ws))
    end
    function unpack(θ)
        L = 3  # Fixed for 3 memory channels
        σ = exp(θ[1]) + 8
        β = exp(θ[2]) + 2.5
        ρ0 = exp(θ[3]) + 20
        dγ = exp.(θ[4:4+L-1])
    ...
    function mean_lyapunov_for_params(σ, β, ρ0, γs, ws; tspan=(0.0, 120.0), saveat=0.1, transient=30.0, u0_set=default_u0_set(length(γs)), solver_hint=:stiff, opt_mode=false)
        ...
        if opt_mode
            ...
            Threads.@threads for i in eachindex(u0_set)
                λs[i] = lyapunov_direct(γs, ws, u0_set[i]; σ=σ, β=β, ρ0=ρ0, tspan=tspan, transient=transient, seed=1234 + i)
                if !isfinite(λs[i])
            ...
            for (i, u0) in enumerate(u0_set)
                λ_i = try
                    sol_i, p_i = run_lorenz(γs, ws; σ=σ, β=β, ρ0=ρ0, tspan=tspan, saveat=saveat, u0=u0, solver_hint=solver_hint)
    ...
    function loss(θ; tspan=(0.0, 120.0), transient=30.0)
        σ, β, ρ0, γs, ws = unpack(θ)
        # Box constraints: Tighter bounds to prevent extreme σ shifts
        if σ < 9.5 || σ > 10.5 || β < 2.6 || β > 2.75 || ρ0 < 27.0 || ρ0 > 29.0
            return 1e3
        end
        if any(!isfinite, γs) || any(!isfinite, ws)
            return 1e3
        end
        λ_mean, _ = mean_lyapunov_for_params(σ, β, ρ0, γs, ws; tspan=tspan, transient=transient, solver_hint=:stiff, opt_mode=true)
        if !isfinite(λ_mean)
            return 1e3
        end
        reg_w = 1e-3 * sum(ws)
    ...
    stage1_iters = quick_mode ? 6 : 15
    stage2_tspan = quick_mode ? (0.0, 140.0) : (0.0, 200.0)
    stage2_transient = quick_mode ? 35.0 : 50.0
    stage2_iters = quick_mode ? 5 : 10
    train_u0 = default_u0_set(L)  # Multi-IC training: all 3 ICs
    val_u0 = default_u0_set(L)

    println("Optimization mode (EXTENDED): ", quick_mode ? "quick" : "full")
    println("  Parameters: σ, β, ρ₀, γ₁, γ₂, γ₃, w₁, w₂, w₃ (8 total)")
    ...
    println("  train_u0_count: ", length(train_u0), " IC(s) [MULTI-IC]")
    suggested = (stage1_iters + stage2_iters) * 10 * length(train_u0)
    println("  expected_loss_evals: ~", suggested, " total")
    # Extended param space needs more evals
    eval_limit[] = 800
    println("  eval_limit set to: ", eval_limit[], " [EXTENDED BUDGET: 8D space + multi-IC + tight box]")
    ...
    σ_opt, β_opt, ρ0_opt, γ_opt, w_opt = unpack(res.u)

    λ_train_mean, λ_train_all = mean_lyapunov_for_params(σ_opt, β_opt, ρ0_opt, γ_opt, w_opt; tspan=stage2_tspan, transient=stage2_transient, u0_set=train_u0, solver_hint=:stiff)
    λ_val_mean, λ_val_all = mean_lyapunov_for_params(σ_opt, β_opt, ρ0_opt, γ_opt, w_opt; tspan=(0.0, 260.0), transient=70.0, u0_set=val_u0, solver_hint=:stiff)

    println("Optimized parameters (EXTENDED):")
    println("  σ = ", round(σ_opt, digits=4), " (baseline: 10.0)")
    println("  β = ", round(β_opt, digits=4), " (baseline: ", round(8/3, digits=4), ")")
    println("  ρ₀ = ", round(ρ0_opt, digits=4), " (baseline: 28.0)")
...
begin
    sol_opt, p_opt = run_lorenz(γ_opt, w_opt; σ=σ_opt, β=β_opt, ρ0=ρ0_opt, tspan=(0.0, 200.0), saveat=0.1)
